...
def expect():
  ...
  try BasicBlock(#1 range: (2, 14), flags=[4, 8, 11], follow_offset=16, edge_count=2, jumps=[36])
        ...
        sequence BasicBlock(#4 range: (36, 38), flags=[6], follow_offset=40, edge_count=1)
        end sequence
      end end-finally
    end except
  end try
end finally
sequence BasicBlock(#5 range: (40, 48), flags=[1, 12], follow_offset=None, edge_count=0)
end sequence
"""
